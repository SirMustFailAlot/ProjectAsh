...
object ServerSpecialChecks : PAServerSubcommand {
    ...
    override fun build(): LiteralArgumentBuilder<CommandSourceStack> =
        literal("Special")
            ...
            .then(literal("Check").executes { ctx ->
                ...
                if (rules.isEmpty()) {
                    ...
                    val lines = buildString {
                        appendLine("Server special targets:")
            ...
            // /Projectash Server Special Clear
            .then(literal("Clear")
                .executes { ctx ->
                    val clearRules = Config.clearServerSpecialRules()
                    if (clearRules) {
                        ctx.source.sendSuccess({ Component.literal("[Project Ash] Server Specials Cleared!") }, true)
                        1
                    } else {
                        ctx.source.sendFailure(Component.literal("[Project Ash] Server Specials Failed to Clear!"))
                        0
                    }
                })
